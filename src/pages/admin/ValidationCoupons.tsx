...
import { Plus, Trash2, Edit, Save, X, Gift } from 'lucide-react';
...
const ValidationCoupons = () => {
  ...
  const handleDeleteCoupon = (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir este cupom?')) {
      deleteCoupon.mutate({ id, type: 'validation' });
    }
  };

  if (isLoading) return <div className="p-6">Carregando cupons de validação...</div>;
      ...
      <Card>
        ...
        <CardContent>
          {!coupons || coupons.length === 0 ? (
            <p className="text-gray-500 text-center py-8">Nenhum cupom de validação encontrado.</p>
          ) : (
